    ...
    <KeyboardAvoidingView style={styles.keyboard} behavior={Platform.OS === "ios" ? "padding" : undefined}>
      <Screen scroll>
        <View style={styles.hero}>
          ...
          <Body>Track your strength locally on this device.</Body>
        </View>
        ...
        <Panel>
          ...
          {isRegistering && password && confirmPassword && password !== confirmPassword ? <Body style={styles.errorText}>Passwords do not match.</Body> : null}
          {authError ? <Body style={styles.errorText}>{authError}</Body> : null}

          <Pressable style={styles.primaryButton} onPress={handleSubmit}>
            {isRegistering ? <UserPlus size={19} color={palette.surface} /> : <LogIn size={19} color={palette.surface} />}
            <Body style={styles.primaryButtonText}>{isRegistering ? "Create account" : "Login"}</Body>
          </Pressable>
        </Panel>
      </Screen>
